...
def process_slice(obj, data, dimfunc, outport):
  ...
  # TODO: combine data into 4D array and output from widget (not to viewport)
  h, w = red.shape[:2]
...
class ExternalNode(gpi.NodeAPI):
  ...
  def compute(self):
    ...
    # construct args
    args = (
      [self, sagittalSlice, 0, 'sagittal slice'], [self, coronalSlice, 0, 'coronal slice'],
      [self, transverseSlice, 0, 'transverse slice']
    )
    ...
    # greenRegion = process_slice(self, coronalSlice, dimfunc=0, outport='coronal slice')
