...
sys.path.append('W:\\Mayo\\envs\\gpi_env\\lib\\site-packages\\gpi_core\\shapers\\GPI')

def child_process():
    # TODO What's the path here
    print('PYTHONPATH (child): ' + str(sys.path))
    total = 0
    for i in range(1000):
...
def parent_process():
    # TODO What's the path here
    print('PYTHONPATH (parent): ' + str(sys.path))
    ...
    try:
        ...
        # print(__name__)
        # new_name = __name__
        # if __name__ == new_name:
        # print(3)
        for i in range(thread_count):
            x = multiprocessing.Process(target=child_process)
            processes.append(x)
            x.start()
